...
internal static class InitManager
{
    ...
    internal static void UpdatePlugin()
    {
        UpdateTools();
        UpdateShovelRecipe();
        TerrainTools.Instance.Config.Save();
    ...
    internal static GameObject MakeToolPiece(ToolDB toolDB)
    {
        ...
        GameObject toolPrefab = PrefabManager.Instance.CreateClonedPrefab(toolDB.name, toolDB.basePrefab);
        if (toolPrefab == null) { return null; }

        // customize piece component
        Piece toolPiece = toolPrefab.GetComponent<Piece>();
        ...
        {
            Transform ghost = toolPrefab.transform.Find("_GhostOnly");
            if (ghost != null)
            {
    ...
    private static void RegisterPieceInPieceTable(GameObject prefab, string pieceTable, string category, int position = -1)
    {
        ...
        PieceTable table = PieceManager.Instance.GetPieceTable(pieceTable);
        if (table == null)
        {
    ...
    private static void RemovePieceInPieceTable(ToolDB toolDB)
    {
        ...
        PieceTable table = PieceManager.Instance.GetPieceTable(toolDB.pieceTable);
        if (table == null)
        {
        ...
        int pos = table.m_pieces.IndexOf(prefab);
        if (pos < 0)
        {
            return;
        }
        ...
        List<int> insertIds = InsertionIndexes[toolDB.pieceTable];
