...
internal static class Shovel
{
    ...
    {
        GameObject shovel = PrefabManager.Instance.CreateClonedPrefab(ShovelPrefabName, "Hoe");

        // Get all the child objects I want to alter
        GameObject handleCollider = shovel?.transform?.Find("collider")?.gameObject;
        GameObject bladeCollider = shovel?.transform?.Find("collider (1)")?.gameObject;
        GameObject attach = shovel?.transform?.Find("attach")?.gameObject;
        GameObject visual = attach?.transform?.Find("visual")?.gameObject;
        ...
        Vector3 visualPos = attach.transform.localPosition + visual.transform.localPosition;
        Quaternion visualRot = attach.transform.localRotation * visual.transform.localRotation;
        // turn blade into iron shovel head
        try
        {
            GameObject blade = visual?.transform?.Find("blade")?.gameObject;
            ...
            // change material
            MeshRenderer bladeRender = blade.GetComponent<MeshRenderer>();
            ...
            BoxCollider boxCollider1 = bladeCollider.GetComponent<BoxCollider>();
        ...
        try
        {
            ...
            GameObject handle = visual?.transform?.Find("handle")?.gameObject;
            ...
            BoxCollider boxCollider = handleCollider.GetComponent<BoxCollider>();
        ...
        try
        {
            if (shovel.TryGetComponent(out ItemDrop itemDrop))
            {
                ...
                if (sharedData != null)
                {
                    Texture2D icon = IconCache.Shovel;
